...
and checkExp  (ftab : FunTable)
    ...
    *)
    ...
    (* TODO project task 1:
        Implement by pattern matching Plus/Minus above.
        See `AbSyn.fs` for the expression constructors of `Equal`, ... *)
    ...
    *)
    | Replicate (n_exp, a_exp, _, pos) ->
        ...
        if a_type <> a_type then
          reportTypesDifferent "second argument of replicate" a_type a_type pos
    ...
    *)
    // checks that elements in arr hgave type that goes into f
