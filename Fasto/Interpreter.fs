...
let rec evalExp (e : UntypedExp, vtab : VarTable, ftab : FunTable) : Value =
  ...
  | Times(e1, e2, pos) ->
   ...
   | Divide(e1, e2, pos) ->
      ...
      let res2 = evalExp(e2, vtab, ftab)
      match (res1, res2) with
            | (IntVal n1, IntVal n2) -> if n2 = 0
                                          then raise (MyError("Division by zero", pos))
                                          else IntVal (n1/n2)
            | (IntVal _, _) -> reportWrongType "right operand of /" Int res2 (expPos e2)
            | (_, _) -> reportWrongType "left operand of /" Int res1 (expPos e1)
  ...
  *)
  | And (e1, e2, pos) ->
      let res1 = evalExp(e1, vtab, ftab)
      match res1 with
        | BoolVal b1 -> if b1 = false then BoolVal false else evalExp(e2, vtab, ftab)
        | _ -> reportWrongType "left operand of &&" Bool res1 (expPos e1)
  | Or (e1, e2, pos) ->
      let res1 = evalExp(e1, vtab, ftab)
      match res1 with
            | BoolVal b1 -> if b1 = true then BoolVal true else evalExp(e2, vtab, ftab)
            | _ -> reportWrongType "left operand of ||" Bool res1 (expPos e1)
  | Not(e1, pos) ->
        let res1 = evalExp(e1, vtab, ftab)
        match res1 with
